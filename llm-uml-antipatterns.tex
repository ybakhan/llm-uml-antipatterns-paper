...
database system used to merge specimen collections held by the University of
Colorado, the Denver Museum of Nature and Science, and the Denver Botanic
Gardens~\cite{mapstedi}.  The UC model was originally prepared by
graduate students in the Ecology and Evolutionary Biology Department at the
University of Colorado and has been used as a benchmark in prior antipattern
detection research~\cite{elattar2006matching,elattar2010improving}.  The
...
\begin{figure}[h]
  ...
  \caption{MAPSTEDI subsystem UC diagram used as the real-world evaluation input.}
  \label{fig:mapstedi}
\end{figure}

Figure~\ref{fig:mapstedi} shows the UC diagram.  The three ground-truth
...
instance~(1).  The 4~FPs expose three failure modes already
observed in the synthetic test set but compounded by the diagram's higher
complexity: two FPs flag use cases that carry direct actor associations
(\textit{Search Collections Data} associated with Public and Research Users;
\textit{Edit Collections Data} associated with Database Integrator and Data
Editor), repeating the actor-association check failure documented in
Section~\ref{sec:evaluation}; one FP flags \textit{Query Remote Database},
which is included by \textit{two} base use cases and therefore represents
legitimate shared behaviour rather than functional decomposition; and one FP
flags \textit{RunQC Tests} despite it having an outgoing include to
\textit{Query Remote Database}, violating the criterion that a decomposed use
case must not itself include other use cases.  The 2~FNs,
